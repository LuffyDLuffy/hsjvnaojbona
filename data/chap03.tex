...
\begin{table}[!htbp]
    ...
    \hline
    0                   & $0(t_1+t_2+t_3+t_4)$       & $\beta_{\rm{h}}^{-1}$  & $\lambda_0$   \\ 

    1                   & $t_1$   & $\beta_{\rm{h}}^{-1}$ & $\lambda_1$   \\ 

    2                   & $t_1+t_2$   & $\beta_{\rm{c}}^{-1}$ & $\lambda_2$   \\ 

    3                   & $t_1+t_2+t_3$   & $\beta_{\rm{c}}^{-1}$ & $\lambda_3$   \\ 
    \hline
    \end{tabular}
    \label{t3.1}
\end{table}
...
\begin{equation}
    \frac{\lambda_2}{\beta_{\rm{c}}} =\frac{\lambda_1}{\beta_{\rm{h}}}
    \label{eq3.16.5}
\end{equation}
绝热捷径B过程中的能量差$\Delta E_{\rm{B}}$吸收热$Q_{\rm{B}}$，输入功$W_{\rm{B}}$正如本章开始所讨论的那样，
\begin{equation}
...
\begin{equation}
    \Delta E_{\rm{C}}=0,\ -W_{\rm{C}}=Q_{\rm{C}} = \beta_{\rm{c}}^{-1} \ln{\frac{\lambda_3}{\lambda_2}} - \beta_{\rm{c}}^{-1} \gamma \int_{t_1 + t_2}^{t_1 + t_2+t_3} \mathrm{d} t    \dot{\lambda}^2 
    \label{eq3.18}
\end{equation}
...
\begin{equation}
    \frac{\lambda_3}{\beta_{\rm{c}}} =\frac{\lambda_0}{\beta_{\rm{h}}}
    \label{eq3.18.5}
\end{equation}
而且和B过程类似，不难得到
\begin{equation}
...
\begin{table}[!htbp]
    ...
    \hline
    C    & 0       & $-\beta_{\rm{c}}^{-1} \ln{\frac{\lambda_3}{\lambda_2}} + \beta_{\rm{c}}^{-1} \gamma \int_{t_1 + t_2}^{t_1 + t_2+t_3} \mathrm{d} t    \dot{\lambda}^2 $                    &$\beta_{\rm{c}}^{-1} \ln{\frac{\lambda_3}{\lambda_2}} - \beta_{\rm{c}}^{-1} \gamma \int_{t_1 + t_2}^{t_1 + t_2+t_3} \mathrm{d} t    \dot{\lambda}^2 $     \\ 
    \hline
...
\begin{equation}
    \begin{split}
        ...
        &=1+ \frac{\beta_{\rm{c}}^{-1} \ln{\frac{\lambda_3}{\lambda_2}} - \beta_{\rm{c}}^{-1} \gamma \int_{t_1 + t_2}^{t_1 + t_2+t_3} \mathrm{d} t    \dot{\lambda}^2 }{\beta_{\rm{h}}^{-1} \ln{\frac{\lambda_1}{\lambda_0}} - \beta_{\rm{h}}^{-1} \gamma \int_{0}^{t_1} \mathrm{d} t    \dot{\lambda}^2}
    \end{split}
    \label{3.20}
\end{equation}
而功率$P$为
\begin{equation}
    \begin{split}
        P&=-\frac{W}{t_1+t_2+t_3+t_4}\\
        &=\frac{Q_{\rm{A}} + Q_{\rm{C}}}{t_1+t_2+t_3+t_4}\\
        &=\frac{\left(\beta_{\rm{h}}^{-1}-\beta_{\rm{c}}^{-1}\right) \ln{\frac{\lambda_1}{\lambda_0}} - \left(\beta_{\rm{h}}^{-1} \gamma \int_{0}^{t_1} \mathrm{d} t    \dot{\lambda}^2 + \beta_{\rm{c}}^{-1} \gamma \int_{t_1 + t_2}^{t_1 + t_2+t_3} \mathrm{d} t    \dot{\lambda}^2\right)}{t_1+t_2+t_3+t_4}
    \end{split}
    \label{eq3.21}
\end{equation}
其中最后一步用到了式\eqref{eq3.16.5}和式\eqref{eq3.18.5}。

现在考虑使得功率$P$最大时的效率$\eta$，对于上式\eqref{eq3.21}分子的第二项，除去一个负号外始终是大于零的。我们考虑选取适当的$\lambda(t)$的形式，使得积分$\int_{t_0}^{t_1} \mathrm{d} t    \dot{\lambda}^2$和$\int_{t_2}^{t_3} \mathrm{d} t    \dot{\lambda}^2$取最小值。这一点可以使用变分方法求得
\begin{equation}
    \ddot{\lambda}=0
    \label{eq3.22}
\end{equation}
可见函数$\lambda=\lambda(t)$是一个线性关系，于是不难得到功率$P$的表达式
\begin{equation}
    \begin{split}
        P&=\frac{\left(\beta_{\rm{h}}^{-1}-\beta_{\rm{c}}^{-1}\right) \ln{\frac{\lambda_1}{\lambda_0}} - \left(\beta_{\rm{h}}^{-1} \gamma (\lambda_1 - \lambda_0)^2 \frac{1}{t_3} + \beta_{\rm{c}}^{-1} \gamma (\lambda_3 - \lambda_2)^2 \frac{1}{t_1}\right)}{t_1+t_2+t_3+t_4}\\
        &=\frac{\left(\beta_{\rm{h}}^{-1}-\beta_{\rm{c}}^{-1}\right) \ln{\frac{\lambda_1}{\lambda_0}}-\gamma\left(\lambda_{1}-\lambda_{0}\right)^{2} \beta_{\mathrm{h}}^{-2}\left(\beta_{\mathrm{h}} \frac{1}{t_{1}}+\beta_{\mathrm{c}} \frac{1}{t_{3}}\right)}{t_1+t_2+t_3+t_4}
    \end{split}
    \label{eq3.23}
\end{equation}
现在优化$t_1,\ t_3$使得$P$最大，只需使$\PP{P}{t_1}=\PP{P}{t_3}=0$即可解得
\begin{equation}
    {\beta_{\mathrm{h}}}{t_1^2}={\beta_{\mathrm{c}}}{t_3^2}
    \label{eq3.24}
\end{equation}
于是，效率可以写为
\begin{equation}
    \begin{split}
        \eta&=1-\frac{\beta_{\rm{c}}^{-1}\ln{\frac{\lambda_1}{\lambda_0}+
        ''\beta_{\rm{c}}^{-1} \gamma (\lambda_1 - \lambda_0)^2 \frac{1}{t_1}}}{\beta_{\rm{h}}^{-1}\ln{\frac{\lambda_1}{\lambda_0}-\beta_{\rm{h}}^{-1} \gamma (\lambda_3 - \lambda_2)^2 \frac{1}{t_3}}}\\
        &=1-\frac{\beta_{\rm{c}}^{-1}}{\beta_{\rm{h}}^{-1}}\frac{\ln{\frac{\lambda_1}{\lambda_0}- \gamma (\lambda_1 - \lambda_0)^2 \frac{1}{t_1}}}{\ln{\frac{\lambda_1}{\lambda_0}+\left(\frac{\beta_{\rm{c}}}{\beta_{\rm{h}}}\right)^{5/2} \gamma (\lambda_1 - \lambda_0)^2 \frac{1}{t_1}}}
    \end{split}
    \label{eq3.25}
\end{equation}
