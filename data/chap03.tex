...
\qquad 考虑一个和温度为$T$的热浴接触的一维布朗粒子。它被施加一个依赖于时间的外势$U(q,p,t) = U_0 (q,\lambda(t)) + U_1 (q,t)$，其中考虑到了在绝热捷径和等温捷径中我们施加的辅助势$U_1$。在过阻尼情况下，粒子的惯性的影响可以忽略，哈密顿量为
\begin{equation}
    H=U_0 (q,\lambda(t)) + U_1 (q,p,t)
    \label{eq31.1}
\end{equation}
重复\ref{c1s1.3.2}随机能量学的讨论，我们容易得到如下的定义，能量差
\begin{equation}
...
\begin{equation}
    \bar{q} \equiv \int_{t_{i}}^{t_{f}} \M{d} t \dot{q} \frac{\partial U}{\partial q} .
    \label{eq31.5}
\end{equation}
并且，显然对于每条相轨有
\begin{equation}
...
而系统的分布函数$\rho (q,p,t)$由的福克-普朗克方程\eqref{eq2.45}决定,在知道了$\rho (q,p,t)$之后就可以计算以上式子\eqref{eq31.4}-\eqref{eq31.6}的系综平均，这和文献\cite{Seifert2005,Shizume1995,Bizarro2011}的步骤是类似的。得到能量差和输入功的系综平均为
\begin{equation}
...
而系统的分布函数$\rho (q,p,t)$由推广的福克-普朗克-克拉马斯方程\eqref{eq2.57}决定。接下来开始计算以上式子\eqref{eq3.4}-\eqref{eq3.6}的系综平均，能量差和输入功的系综平均为
\begin{equation}
...
第一步，对系综中在$t$时刻经过$x$且动量为$p$的轨道进行平均
\begin{equation}
...
\begin{equation}
    ...
    \label{eq3.11}
\end{equation}
再将流矢量$\bm{J}$\eqref{eq2.57.5}的定义代入上式\eqref{eq3.11}，不难得到
\begin{equation}
...
在绝热捷径中，由于系统不与任何热浴接触，故没有热交换，$Q=0,\ \Delta E = W$。由于在绝热捷径中，系统的初末状态依然是正则平衡态。初末的分布函数分别为式\eqref{eq2.32}和式\eqref{eq2.34}，所以，我们依然可以用式\eqref{eq3.7}来计算能量差，于是
\begin{equation}
    \begin{split}
        W &= \Delta E\\
...
在上图\ref{p3.1}中，由实线连接的$0 \to 1$代表了由等温捷径实现的“等温膨胀”过程。正如在第\ref{sec2.3.2}节所阐释的，参数$\lambda$表征了系统的空间尺度，而该过程中参数$\lambda$变大了，故把A过程称为等温膨胀过程。在A过程中，系统与高温热源$\beta_\mathrm{h}^{-1}$接触。可以由式\eqref{eq3.15}计算A过程系统吸收的能量，又鉴于在这个过程中由式\eqref{eq3.13}知有$\Delta E_{\mathrm{A}} = W_{\mathrm{A}} + Q_{\mathrm{A}}=0$，所以
\begin{equation}
    \Delta E_{\mathrm{A}}=0,\ -W_{\mathrm{A}}=Q_{\mathrm{A}} = \beta_{\mathrm{h}}^{-1} \ln{\frac{\lambda_1}{\lambda_0}} - \beta_{\mathrm{h}}^{-1} C_{\mathrm{h}} [\lambda_{\M{h}}(\tau)] \frac{1}{t_1}     
    \label{eq3.16}
\end{equation}
...
\begin{equation}
    ...
    \label{eq3.16.5}
\end{equation}
绝热捷径B过程中的能量差$\Delta E_{\mathrm{B}}$，吸收热$Q_{\mathrm{B}}$，输入功$W_{\mathrm{B}}$正如本章开始所讨论的那样，分别为
\begin{equation}
...
现在考虑功率$P$，一个循环的做功和绝热过程无关，但功率却与绝热过程的时间成反比。但我们对绝热过程的时间并没有限制，原则上这个时间可以任意短。所以的讨论中只考虑$t_2 + t_4 \ll t_1 + t_3$时的情况，或者说和微观随机热机模型一样\cite{Schmiedl2008}，我们忽略绝热过程的时间
\begin{equation}
    \begin{split}
        P&=-\frac{W}{t_1+t_3}\\
...
现在唯一的问题就是如何把$\chi$用$\eta_{\mathrm{c}}$表示出来，这一点可以这样考虑。对于上式\eqref{eq3.35}分子的第二项，除去一个负号外始终是大于零的。既然我们关心功率最大时的效率，就应当考虑选取适当的$\lambda_{\mathrm{c}}(\tau),\ \lambda_{\mathrm{h}}(\tau)$的形式，使得关于它们的泛函$C_{\mathrm{c}} [\lambda_{\mathrm{c}}(\tau)],\ C_{\mathrm{h}} [\lambda_{\mathrm{h}}(\tau)]$都取最小值。这一点可以使用变分的方法求得\footnote{不要忘记，我们对$\lambda(t)$的限制条件\eqref{eq2.51},在一般情况这个附加的条件会导致$\lambda(t)$无解。但我们可以这么考虑，先不考虑限制条件\eqref{eq2.51}，解出$\lambda(t)$。接下来，将函数$\lambda(t)$靠近边界$\lambda_i,\ \lambda_f$的部分“磨平”，使得它满足限制条件\eqref{eq2.51}。在欠阻尼情形下我们也将采用这样的策略。}，分别得到
\begin{equation}
    \DDD{\lambda_{c}}{\tau}=0,\quad \DDD{\lambda_{h}}{\tau}=0
    \label{eq3.38}
\end{equation}
...
\begin{equation}
    ...
    \label{eq3.41}
\end{equation}
可以看到，这符合热机最大功率时效率的二阶普适性，而且很有意思的是它没有三阶项。
...
\begin{equation}
    ...
    \label{eq3.47}
\end{equation}
其中$C_{\mathrm{h0}}$是不依赖于$\tau$的常数。非常有意思的是，我们发现，上式\eqref{eq3.47}的左边恰好是式\eqref{eq3.46}里的积分项。那也就意味着
\begin{equation}
...
\begin{align}
    ...
    \label{eq3.50}
\end{align}
考虑到第\ref{c1s3}节提到的欠阻尼福克-普朗克方程的适用条件\cite{Sekimoto2010}，而对于谐振子势场来说，$\lambda$正是系统的特征时间尺度，于是有
\begin{equation}
    \gamma \lambda \ll 1,\quad \lambda \text{可取}\lambda_0,\ \lambda_1,\ \lambda_2,\ \lambda_3
    \label{eq3.51.5}
\end{equation}
于是忽略$\gamma \lambda$的二阶项，可得
\begin{equation}
...
\begin{equation}
    ...
    \label{eq3.52}
\end{equation}
于是将上式\eqref{eq3.52}代入最大功率对应的效率的表达式\eqref{eq3.45}，热机最大功率对应的效率
\begin{equation}
    \eta=1-\sqrt{1-\eta_{\mathrm{c}}}
    \label{eq3.53}
\end{equation}
这正是Curzon 和 Ahlborn 构建的内可逆热机\cite{Curzon1975}的效率$\eta_{\mathrm{CA}}$，也和涂展春构建的类卡诺热机一致。\cite{Tu2013}
