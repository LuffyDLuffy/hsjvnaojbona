...
现在唯一的问题就是如何把$\chi$用$\eta_{\mathrm{c}}$表示出来，这一点可以这样考虑。对于上式\eqref{eq3.35}分子的第二项，除去一个负号外始终是大于零的。既然我们关心功率最大时的效率，就应当考虑选取适当的$\lambda_{\mathrm{c}}(\tau),\ \lambda_{\mathrm{h}}(\tau)$的形式，使得关于它们的泛函$C_{\mathrm{c}} [\lambda_{\mathrm{c}}(\tau)],\ C_{\mathrm{h}} [\lambda_{\mathrm{h}}(\tau)]$都取最小值。这一点可以使用变分的方法求得\footnote{不要忘记，我们对$\lambda(t)$的限制条件\eqref{eq2.51},在一般情况这个附加的条件会导致$\lambda(t)$无解。但我们可以这么考虑，先不考虑限制条件\eqref{eq2.51}，解出$\lambda(t)$。接下来，将函数$\lambda(t)$靠近边界$\lambda_i,\ \lambda_f$“磨平”。在欠阻尼情形下我们也采用这种策略。}，分别得到
\begin{equation}
