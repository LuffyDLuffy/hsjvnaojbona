...
\begin{equation}
    ...
    \label{eq2.31}
\end{equation}
在时间$t_i < t \leq t_f$内，参数由$\lambda_{i} \equiv \lambda(t_i)$变为$\lambda_{f} \equiv \lambda(t_f)$。我们要求$\dot{\lambda}\left(t_{i}\right)=\dot{\lambda}\left(t_{f}\right)=0$，这样才有$H\left(t_{f}\right)=H_0 \left(t_{f}\right),\ H\left(t_{f}\right)=H_0 \left(t_{f}\right)$，\R{（不特别清楚，大概是为了使初态和末态保持一致？）}除此之外，$\lambda (t)$依赖于时间$t$的形式有很大的任意性。
...
\begin{equation}
    ...
    \label{eq2.33}
\end{equation}
接着我们找到末态的有效温度，使得末态的分布函数\eqref{eq2.33}可以被表达成正则分布的样子
\begin{equation}
...
这样，通过绝热捷径，在时间$t_i \sim t_f$内，我们实现了终于微观正则态之间的转换。
