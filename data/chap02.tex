...
\begin{equation}
    ...
    \label{eq2.43}
\end{equation}
其中，$F(\lambda) \equiv-\beta^{-1} \ln \left[\int e^{-\beta H_{0}(\Gamma, \lambda)} d \Gamma\right]$是原系统在给定参数$\lambda$时，在平衡态下的自由能。\eqref{eq2.43}的积分是对整个相空间的。如果我们限制辅助势$U_1 (\Gamma ,\ t)$在驱动过程的初末时刻都为零，即分布函数在初末时刻都对应于哈密顿量$H(\Gamma \, t)$，那么我们就实现了有限时间内的等温平衡态的转化。
