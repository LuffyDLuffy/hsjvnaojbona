...
和欠阻尼的情况一样，依然考虑两个特殊的势场\eqref{eq2.52}和\eqref{eq2.52}。可以求得相应的辅助势为
\begin{equation}
...
\begin{equation}
    ...
    \label{eq2.62}
\end{equation}
类似于过阻尼的情形，在欠阻尼的情形下我们也可以为两类势场的辅助势找到解析解\cite{Li2016}，势场$U_0 (q, \lambda(t))= \lambda(t) q^{n} / 2 \quad (n=2,4,6\cdots) ,\ U_{0}(q, \lambda(t))=u(q-\lambda(t))$的辅助势分别为$U_{1}(q, p, t)\dot{\lambda}\left[(p-\gamma q)^{2}+\lambda(t) q^{n}\right] / 2 n \gamma \lambda ,\ U_{1}(q, p, t)=\dot{\lambda}(p-\gamma q)$。
