...
考虑到我们对辅助势的$U(q,t)$限制条件——在驱动过程的初末时刻$t_i , t_f$要使得$U(q,t)=U_0 (q,t)$，于是我们引入边界条件
\begin{equation}
...
\begin{equation}
    U_{1}(x, t)=-\frac{\gamma \dot{\lambda}(t)}{k} x
    \label{eq2.53}
\end{equation}
再来看一个例子：依赖于时间的谐振子势场中的布朗粒子，势场为
\begin{equation}
    U_{0}(x, \lambda(t))=\frac{1}{2} \lambda(t) x^{2}
    \label{eq2.54}
\end{equation}
同样的，根据式\eqref{eq2.50}可得辅助势为
\begin{equation}
    U_{1}(x, t)=\frac{\gamma \dot{\lambda}(t)}{4 \lambda(t)} x^{2}
    \label{eq2.55}
\end{equation}
\eqref{eq2.55}和Martínez等人得到的结果\cite{Martinez2016}是等价的。并且，他们的结果意味着对于势场$U_0 (q, \lambda(t))= \lambda(t) x^{n} / 2 \quad (n=2,4,6\cdots)$都可以找到辅助势的解析解$U_{1}(x, t)=\gamma \dot{\lambda} x^{2} /(2 n \lambda)$。除此之外，也可以找到另一类势场$U_{0}(x, \lambda(t))=u(x-\lambda(t))$的辅助势，这是一个随时间平移的势场，相应的辅助势为$U_{1}(x, t)=-\gamma \dot{\lambda} x$\cite{Li2016}。福克-普朗克-克拉马斯方程只能描述势场不依赖于动量$p$时分布函数$\rho (x,p,t)$的时间演化。
...
\begin{equation}
    H(x, p, t)=H_{0}(x, p, \lambda(t))+U_{1}(x, p, t)
    \label{eq2.44}
\end{equation}
其中$H_{0}(x, p, \lambda(t))=\frac{p^{2}}{2 m}+U_{0}(x, \lambda(t))$
